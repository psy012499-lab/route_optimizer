...
# 실행마다 고유한 출력 경로 생성 → 동시 실행 시 파일 충돌 방지
_run_id      = _uuid.uuid4().hex[:8]
OUTPUT_EXCEL = f"output_result_c7_{_run_id}.xlsx"
...
ALGORITHM_NAME = "CORSE7 (통상코스 유지 · 순로 최적화)"
VERSION        = "1.0.0"
...
def hybrid_road_corrected_route(
    ...
    while unvisited:
        ...
        for cand in candidates:
            d, t = get_road_distance_safe(current, cand)
            if t < best_time:
                best = cand
                best_time = t
